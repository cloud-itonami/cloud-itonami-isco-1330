(ns it-services.actor-test
  (:require [clojure.test :refer [deftest is testing]]
            [it-services.actor :as actor]
...
(deftest commits-a-clean-low-risk-request
  (let [st (fresh-store)
        ...
        request {:client-id "client-1" :op :monitor :stake :low}
...
(deftest holds-on-unregistered-client-without-committing
  (let [st (fresh-store)
        graph (actor/build-graph {:store st})
        request {:client-id "no-such-client" :op :monitor :stake :low}
...
(deftest interrupts-then-commits-on-human-approval
  (let [st (fresh-store)
        graph (actor/build-graph {:store st})
        ;; data-center power-cycle always escalates (governor invariant)
        request {:client-id "client-1" :op :power-cycle-datacenter :stake :high}
