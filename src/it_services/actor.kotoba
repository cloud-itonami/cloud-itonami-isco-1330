(ns it-services.actor
  "ITServicesActor — the ISCO-08 1330 independent IT-services-management
  actor as a `langgraph.graph/state-graph` (per ADR-2607011000 /
  CLAUDE.md Actors section). One graph run = one IT operation request
  ...
  The unconditional invariant: the ITServicesAdvisor can never directly
  commit a record or dispatch a robot action the ITServicesGovernor
  refuses — every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [it-services.advisor :as advisor]
            [it-services.governor :as governor]
            [it-services.store :as store]))

(defn build-graph
  "Build a compiled ITServicesActor graph. `store` implements
  `it-services.store/Store`. `advisor` implements
  `it-services.advisor/Advisor` (defaults to `mock-advisor`).
